...
const zh = {
  ...
  titlebar: {
    ...
    passthrough: (on: boolean) => `透传${on ? '开' : '关'}`,
    passthroughTitle: '真实鼠标事件送到下方网页；透传期间窗口大小锁定，仍可拖动标题栏移动；编辑文字时本软件临时获得键盘焦点',
    passthroughUnsupported: '鼠标透传目前仅支持 Windows',
    passthroughFailed: '鼠标透传启动失败，已恢复普通点击',
  ...
  settings: {
    ...
    answerCustomPromptLabel: '问答自定义提示词',
    answerCustomPromptPlaceholder: '例如：回答尽量简洁，先给结论，再补充关键依据。',
    answerCustomPromptHint: '用于文字问答和截图问答题；编程题与翻译不受影响。留空使用默认规则。',
    audioSection: '音频设备',
...
const en: Dict = {
  ...
  titlebar: {
    ...
    passthrough: (on: boolean) => `Pass:${on ? 'On' : 'Off'}`,
    passthroughTitle: 'Pass physical mouse input to the page below; window size is locked while on, but the title bar can move it; text editing temporarily takes keyboard focus',
    passthroughUnsupported: 'Mouse passthrough currently supports Windows only',
    passthroughFailed: 'Mouse passthrough failed; normal clicks were restored',
  ...
  settings: {
    ...
    answerLangEn: 'English',
    answerCustomPromptLabel: 'Custom Q&A instructions',
    answerCustomPromptPlaceholder: 'For example: Keep answers concise, lead with the conclusion, then give key reasons.',
    answerCustomPromptHint: 'Used for text Q&A and screenshot questions. Coding answers and translation are unchanged. Leave empty for defaults.',
    ...
    opacityHint: 'Lower values make the whole window, including text, more transparent.',
    otherSection: 'Other',
    otherHint:
