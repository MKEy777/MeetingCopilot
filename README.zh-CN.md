...
*真实录屏（非摆拍）：面试官话还没说完，左栏灰色实时字幕已经跟上；说完自动触发右栏回答，内容优先参考第二简历，个人经历由简历补充，可直接照着念。*
...
- 🌍 **中英双语开箱即用**：一场会里中英夹着说，转录自动识别语言切换，不用动任何设置；答案语言点一下 `答:EN` 就切成英文提词，照样优先参考第二简历——外企英文面试直接用。
...
- 📄 **简历 / 第二简历双槽按会话导入**：支持 `.md/.txt/.docx/.pdf`，全部本地确定性解析。第二简历可放八股、常见面试题、技术原理和答题要点，模型优先参考；提问时作为上下文发送给你配置的大模型。自动识别题型并附回答提示。
...
- 🖼️ **完整屏幕截图问答**：一键截取主显示器完整画面，交给视觉模型（MiMo / Gemini）回答。
- 🥷 **采集保护**：内容保护配合全局快捷键隐藏/呼出。Windows 可在受支持的采集方式中排除窗口；macOS 面对新版 ScreenCaptureKit 无法保证完全隐身。
- 🩺 **连接测试、服务状态与本地诊断**：一键分辨是 Key、网络还是账号的问题（14 个统一错误代码）；诊断报告在本机生成，不含 Key、转写和导入的简历/参考资料内容。
...
*先一个中文问题、再一个英文问题——同一会话、零设置改动，本地转录自动识别语言切换（都在 1.6 秒左右上屏）；点一下 `答:EN`，英文答案照样优先参考同一份第二简历。*
...
- 诊断报告在本机生成，不含 Key、转写和导入的简历/参考资料内容，可以直接贴到公开 issue。
...
npm start          # 后台启动，关闭终端后仍继续运行
npm stop           # 关闭后台运行的 MeetingCopilot
# Windows 也可双击 start.bat 启动
```
